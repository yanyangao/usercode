...
echo "| selection | *R* | N(in)-raw |*N(in) (OF/VZ subt)*| N(in) data/MC|* N(out)-data* | N(out) MC | N(out) data/MC|"
...
#for MASS in 115 ; do
    ...
    OUTSCALE=`grep -C 1 'data/MC scale factor' $FILE | tail -n 1 | awk '{print $9""$10""$11}'`
    INSCALE=`grep -C 1 'data/MC scale factor from Z-window ' $FILE | tail -n 1 | awk '{print $8""$9""$10}'`
    echo "| $SELECTION | *$R* | $ZDATA | *$ZDATASUB* | $INSCALE | *$DATADRIVEN* | $MC | $OUTSCALE |"
done
